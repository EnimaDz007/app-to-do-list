...
export const VoiceTaskModal: React.FC<VoiceTaskModalProps> = ({
      ...
      {isOpen && (
        <>
          ...
          <motion.div
            ...
            className="fixed left-1/2 top-1/2 z-50 w-[88%] max-w-sm -translate-x-1/2 -translate-y-1/2 rounded-[28px] bg-white p-7 shadow-[0_30px_80px_rgba(15,23,42,0.4)]"
          >
            <button
              onClick={step === 'speak' ? handleBack : handleCancel}
              className="absolute left-4 top-4 flex h-7 w-7 items-center justify-center rounded-full bg-slate-100 text-slate-600 transition-transform hover:scale-110"
            >
            ...
            <h2 className="mb-1 text-center text-[20px] font-extrabold tracking-tight text-slate-900 dark:text-white">
                    ...
                    {QUADRANTS.map((q) => {
                      ...
                      return (
                        <button
                          ...
                          className={`flex flex-col items-center gap-2 rounded-2xl border-2 ${q.border} ${q.bg} dark:border-slate-700 dark:bg-slate-800 p-4 transition-all hover:scale-[1.03] active:scale-95`}
                        >
                          <div className={`flex h-11 w-11 items-center justify-center rounded-xl bg-gradient-to-br ${q.gradient} shadow-lg`}>
                            <Icon size={22} className="text-white" />
                          </div>
                          <span className={`text-[12px] font-bold ${q.text}`}>
                ...
                >
                  ...
                  <p className={`mb-4 text-center text-[9px] font-extrabold uppercase tracking-[3px] ${theme.text}`}>
                  ...
                  <div className="relative mx-auto mb-3 h-[72px] w-[72px]">
                    {isListening && (
                      <motion.div
                        ...
                        className={`absolute inset-[-8px] rounded-full border-2 ${theme.border}`}
                      />
                    )}
                    <button
                  ...
                  <div className="mb-3 flex justify-center">
                   <div className={`inline-flex items-center gap-1.5 rounded-full border px-3 py-1 text-[10px] font-bold ${theme.bg} dark:bg-slate-700 ${theme.border} dark:border-slate-600 ${theme.text} dark:text-white`}>
  ...
  rows={3}
  dir="ltr"
  className={`w-full resize-none rounded-2xl border-[1.5px] p-3 pr-10 text-left text-[13px] font-medium outline-none ${theme.bg} ${theme.border} ${theme.text}`}
/>
                    {transcript && (
                      <button
                        onClick={handleClear}
                        className={`absolute right-2 top-2 flex h-6 w-6 items-center justify-center rounded-full ${theme.bg} ${theme.text}`}
                        title={t('ptt_clear')}
                      >
                        <Eraser size={13} />
                      </button>
                  ...
                  <p className="mb-3 text-center text-[10px] italic text-slate-400">
                    {t('ptt_release_tip')}
                  </p>
                  ...
                  <button
                    ...
                    className={`mb-2 flex w-full items-center justify-center gap-2 rounded-2xl bg-gradient-to-br ${theme.gradient} py-3.5 text-sm font-extrabold text-white shadow-lg transition-all active:scale-[0.98] disabled:cursor-not-allowed disabled:opacity-50`}
                  >
                  ...
                  <button
                    onClick={handleBack}
                    className={`w-full py-2.5 text-xs font-semibold ${theme.text} transition-opacity hover:opacity-70`}
                  >
                    ← {t('modal_btn_cancel')}
                  </button>
                </motion.div>
